...
if size(xyCell, 2) ~= m.nr
	error('HMSC: Size of xy does not match number of random effects');
end
...
for r = 1:m.nr
	...
	if( ~m.spatial(r) && ~isempty(xy{r}) )
		error( strcat('HMSC: Model is defined with non-spatial factors on effect', [' ' int2str(r)], ', but non-empty locations matrix was passed for this effect') )
	end
end
