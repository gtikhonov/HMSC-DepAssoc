classdef Hmsc < handle
	...
	methods (Access = private)
		[iQg,detQg,rhopw,iWgA,iWgAR,detWgA,alphapwA] = calculateDataParameters(m);
	...
	methods (Static = true)
		...
		alpha = update_alpha(eta,alpha,nf,pi,spatial,iWg,iWgR,detWg,alphapw)
		z = update_z(z, X,Xr,Xs,beta,eta,lambda,etas,lambdas,Y,nf,pi,ncr,dist,sigma,speciesX,includeXs,factorCov)
		sigma = update_sigma(X,Xr,Xs,sigma,beta,eta,lambda,etas,lambdas,nf,pi,ncr,dist,asigma,bsigma,z,spatial,factorCov,speciesX,includeXs)
		beta = update_beta(X,Xr,Xs,z,nf,pi,ncr,eta,lambda,etas,lambdas,gamma,sigma,T,ph,iV,rho,phylogeny,iQg,detQg,outlierspecies,spatial,factorCov,speciesX,includeXs)
